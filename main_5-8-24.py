...
proff = True # sets previosly off to true

def main(volt, temp):
    display.fill(0)
    display.text(str(volt), 45, 10)
    display.text(str(temp), 45, 20)
...
while True:
    ...
    if B_v < 3.3:
        ...
        if disp.value() == 1: # Reads if the display is on
            print("staying on...")
            main(B_v, t) # Displays Voltage(Change to Wattage) & Temperature (Probably will remove this)
